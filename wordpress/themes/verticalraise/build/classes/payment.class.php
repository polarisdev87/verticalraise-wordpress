...
class Payments {
    ...
    public function validate_params($params) {
        ...
        foreach ($required as $key => $req) {
            switch ($req) {
                case 'float':
                    if (is_float($params[$key]) == false) {
                        $flag = 1;
                    }
                    break;
                case 'int':
                    if (is_int($params[$key]) == false) {
                        $flag = 1;
                    }
                    break;
                case 'string':
        ...
        if (isset($_POST['anonymous'])) {
            if ($_POST['anonymous'] != 1 || $_POST['anonymous'] != 0)
    ...
    public function set_params($post_data) {
        ...
        $params['full_name'] = $post_data['fname'] . ' ' . $post_data['lname'];
        ...
        $params['anonymous'] = ( isset($post_data['anonymous']) ) ? $post_data['anonymous'] : 0;

        $params['stripe_connect'] = $post_data['stripe_connect'];
        $params['force_connect'] = $post_data['force_connect'];
        $params['stripe_connect_account'] = $post_data['stripe_account_id'];
        $params['our_fee'] = $post_data['our_fee'];
        $params['rep_code'] = $post_data['rep_code'];
     ...
     *
     */
    private function insert_payment($stripe_charge, $params) {
...
//                }
     ...
     */
     ...
     public static function transferBackFromAccounts( $fundraiser_id, $account_id, $amount, $transfer_type = Stripe_Transfers::TRANSFER_BY_CRON ) {
		...
		try {
			...
			if ( isset( $transfer->amount ) ) {
				...
				$stripe_transfers = new Stripe_Transfers();
				if ( $stripe_transfers->table_exist() ) {
					$stripe_transfers->insert( $fundraiser_id, json_encode( $transfer ), Stripe_Transfers::TRANSFER_SUCCESS, $transfer_type );
				}
				$transferred = true;
			} else {
                $stripe_transfers = new Stripe_Transfers();
				if ( $stripe_transfers->table_exist() ) {
					$stripe_transfers->insert( $fundraiser_id, json_encode( $transfer ), Stripe_Transfers::TRANSFER_ERROR, $transfer_type );
				}
				$transferred = false;
			}
			...
			if ( extension_loaded('newrelic') ) { // Ensure PHP agent is available
				newrelic_notice_error("Fundraiser ID: {$fundraiser_id}");
				newrelic_notice_error($e->getMessage(), $e);
			}
			$stripe_transfers = new Stripe_Transfers();
			if ( $stripe_transfers->table_exist() ) {
				$stripe_transfers->insert( $fundraiser_id, $e->getMessage(), Stripe_Transfers::TRANSFER_ERROR, $transfer_type );
			}
			$transferred = false;
		}
