...
function payout_connect_funds() {
	...
	$query = "SELECT p.ID FROM fundraiser_details f
                  ...
                  WHERE f.transferred = 0 AND p.post_status = 'publish' AND  pm.meta_key = 'stripe_connect' AND pm.meta_value = 1
                  HAVING p.ID IN 
                  ( 
                    SELECT m.post_id FROM wp_postmeta m 
                    WHERE m.meta_key = 'end_date' AND CAST(m.meta_value as DATE) < NOW() 
                  )
              ";
	...
	foreach ( $fundraisers as $fundraiser ) {
		...
		if ( $available_amount > 0 ) {
			$success = \Payments::payoutToAccount( $fundraiser->ID, $account_id, $original_available_amount );
		}
