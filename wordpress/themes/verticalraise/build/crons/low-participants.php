...
function process_low_participants( $period ) {

    try {

        load_class( 'cron.class.php' );

        $cron = new Cron( 'process_low_participants' . $period . 'days', 'l_participant_' . $period . 'days' );
        ...
        if ( $fundraiser_query->have_posts() ) :
            ...
            while ( $fundraiser_query->have_posts() ) : $fundraiser_query->the_post();
                ...
                if ( $end_date == $target_date ) {
                    ...
                    $output = process_low_participants_loop( $fundraiser_id, $end_date, $admins , $period);
                }
 ...
 */
function process_low_participants_loop( $fundraiser_id, $end_date, $admins , $period) {
    ...
    try {
        ...
        if ( !empty( $low_participants ) && count( $low_participants ) > 0 ) {
            ...
            $from    = $admin_email;
            if ( $period === 19 ) {
                $subject = "{$title} - Participation Update";
            } else if ( $period === 10 ) {
                $subject = "Only 10 days left to reach your goal";
            } else {
                $subject = "{$title}";
            }
            $cc      = null;
            ...
            foreach ( $low_participants as $p ) {
                ...
                $response = process_low_participants_player_loop( $p, $template_args, $mail_args );
                if ( $response != 'failed' ) {
 ...
 */
function process_low_participants_player_loop( $p, $template_args, $mail_args ) {
    ...
    try {
        ...
        if ( !empty( $to ) && is_email( $to ) == true ) {
             ...
             * Send the email.
             */
            $sent = $mail->send_api( $to, $from, $cc, $subject, 'low_participants', $template_args, $from_name );
