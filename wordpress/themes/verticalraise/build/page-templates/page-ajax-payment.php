...
//        var_dump("DDDDDD",$charge); exit;
        ...
        try {
	        ...
	        if ( extension_loaded( 'newrelic' ) ) { // Ensure PHP agent is available
		        newrelic_notice_error( $e->getMessage() . ' post data: ' . $post_data, $e );
	        }
	        $err = array(
        ...
        if ( !empty( $charge ) && is_array( $charge ) ) {
            ...
            try {
	             ...
	             * Save to amply
	             */
	            Amply::create_donation( $charge['params']['email'], $charge['params']['full_name'],
		        	$_POST['company_name'], $charge['stripe_charge']['amount'],
		        	$charge['stripe_charge']['transaction_id'], $_POST['fundraiser_id'] );
