<?php
/**
 * Template Name: Donation V3 - Stripe Checkout
 *
...
$author_id = get_post_field( 'post_author', $fid );

$giveAmply_table = new GiveAmply();
$amply_org_id = $giveAmply_table->get_org_id($fid);

if ( $author_id == $uid || in_array_my( $fid, $sadmin ) || $uid == 0 ) {
    $p_supporters_total = $goal->get_num_supporters( $fid );
    $p_supporters       = $goal->get_donators( $fid );
    $p_amount           = $payments->get_total_by_fundraiser_id( $fid );
    $user_type          = 'team';
} else {
    $p_supporters_total = $payments->get_number_supporters_by_user_id( $uid, $fid );
    $p_supporters       = $payments->get_all_payments_by_fundraiser_uid( $uid, $fid );
    $p_amount           = $payments->get_total_by_user_id( $uid, $fid );
...
while ( $fundraiser_query->have_posts() ) : $fundraiser_query->the_post();
    ...
    letter-spacing: 1px; margin-bottom: 0;">Double Your Donation</h3>							<p style="text-align: center;">Enter your employer in the box below and our automated system will send your donation info to them to be matched through their corporate match program</p>
                            <div class="field_row">
                            ...
                            <div class="fb_info">
                                ...
                                <div class="donate_btn">
                                    <button id="payment-button" type="submit" name="continue" data-loading="Continue.."
                                            class="submit_btn has-spinner" tabindex="7">Continue
                                    </button>
                                </div>
    ...
    <script>
        ...
        $("#payment-button").click(function (e) {
            ...
            if( valid ){
                ...
                jQuery.ajax({
                    type: "POST",
                    url: "<?php get_bloginfo( 'url' ); ?>/ajax-payment-v3",
                    data: {
                        create_session: "1",
