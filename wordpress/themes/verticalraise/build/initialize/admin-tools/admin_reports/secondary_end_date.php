...
ini_set('memory_limit', '6048MB');

use \classes\app\admin_reports\Secondary_End_Date_Reports;

/**
 * Page to see Reports of Secondary End Date
 */

?>

<h2>Secondary End Date</h2>
...
<style>
    ...
    #fundraisers_ended_table_wrapper{
        margin-right: 25px; 
        /*overflow-x: scroll;*/
    }
    #fundraisers_ended_table {
        position: relative
    }

    #fundraisers_ended_table_wrapper .dataTables_scroll {
        max-height: 600px;
        overflow-x: auto;
        position: relative;

    }
    #fundraisers_ended_table_wrapper .dataTables_scroll .dataTables_scrollHead{
    ...
    #fundraisers_ended_table_wrapper .dataTables_scroll .dataTables_scrollBody{
...
<form id="submitForm" method="post" action="/wp-admin/admin.php?page=secondary_end_date">
...
//        })
        ...
        $('#fundraisers_ended_table').DataTable({
            "bPageinate": false,
            "searching": false,
            "bFilter": false,
            "bInfo": false,
...
<?php if (isset($_POST)) { 
    ...
    $fundraiser_ended = new Secondary_End_Date_Reports();
    $result           = $fundraiser_ended->get_all_ended_fundraisers();    
    
?>

<p>Current datetime: <?php echo $fundraiser_ended->get_current_time_formatted() ?></p>

<h3>Results</h3>

<!-- Show Database Custom Table Info -->
<table cellpadding="4" cellspacing="0" border="1" class="dataTable unsticky" id="fundraisers_ended_table">
    ...
    <thead >
        <tr>
            ...
            <th class="center-td"><strong>Our Fee</strong></th>
        ...
        if ( !empty($result) ) {
            foreach ( $result as $item ) {
                ...
                <tr>
                    ...
                    <td class=""><a href="<?php echo $item['fundraiser_link'] ?>" target="_blank"><?php echo $item['fname'] ?></a></td>
                    <td ><span class="right-td"><?php echo $item['created_date'] ?></span></td>
                    <td ><span class="right-td"><?php echo $item['start_data'] ?></span></td>
                    <td ><span class="right-td"><?php echo $item['end_date'] ?></span></td>
                    <td ><span class="right-td"><?php echo $item['secondary_end_date'] ?></span></td>
                    <td ><span class="right-td"><?php echo $item['our_fee'] ?></span></td>
                    ...
                    <td ><span class="right-td">$<?php echo $item['secondary_raised'] ?></span></td>
